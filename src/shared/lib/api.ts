...
export const getPostCards = async (limit = 100, preview = false) => {
	...
	return extractMany(resp) as TPostCard[] | undefined;
};
...
export const getAllPostsSlug = async () => {
  ...
  `);
	return extractMany(resp) as { slug: string }[] | undefined;
};
...
export const getPost = async (slug: string, preview = false, more = 0) => {
	...
	return {
		post: extractOne(postResp) as TPost | undefined,
		morePosts: extractMany(morePostResp) as TPostCard[] | undefined,
	};
};
